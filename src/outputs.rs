...
impl<D: Decoder> Decodable<D> for RgbOutPoint {
    fn consensus_decode(d: &mut D) -> Result<RgbOutPoint, Error> {
        ...
        match byte {
            // 0x1 stands for UTXO-based RGB transaction
            0x1 => Ok(RgbOutPoint::UTXO(Decodable::consensus_decode(d)?)),
            // 0x2 stands for address-based RGB transaction
            0x2 => Ok(RgbOutPoint::Vout(Decodable::consensus_decode(d)?)),
...
mod test {
    ...
    use crate::outputs::{RgbOutEntry, RgbOutPoint};
    use bitcoin;
    ...
    fn encode_utxo_outpoint_test() {
        ...
        assert_eq!(
            data[0], 0x1,
            "First byte of serialized UTXO-based outpoint must be equal to 0x1"
        );
        assert_eq!(
            data.len(),
            data_standard.len(),
            "Wrong length of serialized data"
        );
    ...
    fn encode_vout_outpoint_test() {
        ...
        assert_eq!(
            data[0], 0x2,
            "First byte of serialized vout-based outpoint must be equal to 0x1"
        );
        assert_eq!(
            data.len(),
            data_standard.len(),
