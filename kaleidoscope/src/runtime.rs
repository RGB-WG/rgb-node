...
use super::*;
use crate::accounts::{Account, KeyringManager};
use crate::data::DepositTerminal;
...
impl Runtime {
    pub async fn init(config: Config) -> Result<Self, Error> {
        ...
        debug!(
            "Subscribing to bpd notifications on {} ...",
            config.bpd_subscr
        );
...
impl Runtime {
    ...
    async fn get_deposits(
        &self,
        account_tag: &String,
        deposit_types: Vec<commands::bitcoin::DepositType>,
        offset: u32,
        no: u8,
    ) -> Result<Vec<DepositTerminal>, Error> {
        ...
        let index = offset;
        let network: bitcoin::Network = self
            .config
            .network
            .try_into()
        ...
        let data: HashMap<bitcoin::Script, usize> = depo_boxes
            ...
            .map(|(idx, depo)| -> Vec<(bitcoin::Script, usize)> {
                let mut s = vec![];
                if deposit_types.contains(&PKH) {
                    s.push(depo.get_p2pkh_addr(network))
                }
                if deposit_types.contains(&WPKH) {
                    s.push(depo.get_p2wpkh_addr(network))
                }
                s.into_iter()
                    .map(|addr| -> (bitcoin::Script, usize) {
                        (addr.clone().payload.script_pubkey().into_script(), idx)
                    })
                    .collect()
            })
            .flatten()
        ...
        Ok(data
            ...
            .map(|((script, index), list)| {
                ...
                list.into_iter()
                    .map(|info| DepositTerminal {
                        outpoint: bitcoin::OutPoint {
                            txid: info.tx_hash,
                            vout: info.tx_pos as u32,
                        },
    ...
    fn get_genesis_data(&self, contract_id: ContractId) -> Result<Transition, Error> {
        let reader = self
            .config
            .data_reader(DataItem::ContractGenesis(contract_id))?;
...
impl Runtime {
    ...
    pub fn account_create(
        ...
        info!(
            "Creating new account {} with derivation path {}",
        ...
        self.keyrings
            .store(self.config.data_path(DataItem::KeyringVault))?;
        println!(
            "New account {} successfully added to the default keyring and saved to the vault",
            name
        );
        Ok(())
    }

    pub fn account_deposit_boxes(
        self,
        account_tag: String,
        offset: u32,
        no: u8,
        ...
        println!(
            "{:>4}:  {:64}    {:32}    {:48}",
            "ID", "PUBKEY", "P2WPKH ADDRESS", "P2PKH ADDRESS"
        );
        self.keyrings
            .get_main_keyring()
            .list_deposit_boxes(&account_tag, offset, no)
            ...
            .for_each(|depo| {
                println!(
                    ...
                    index,
                    depo.get_pubkey(),
                    depo.get_p2wpkh_addr(network),
                    depo.get_p2pkh_addr(network)
                );
                index += 1;
            });
        Ok(())
    ...
    pub async fn bitcoin_funds(
        ...
        let deposits = self
            .get_deposits(&account_tag, deposit_types, offset, no)
            .await?;

        println!(
            "{:>4}:   {:^16}  |  {:^64}  |  {:^5}  |  {}",
            "ID", "VALUE", "TXID", "VOUT", "TYPE"
        );
        deposits.into_iter().for_each(|depo| {
